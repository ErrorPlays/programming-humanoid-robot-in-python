...
class PIDController(object):
    ...
    def control(self, target, sensor):
        ...
        '''
        # YOUR CODE HERE
        error_e = target - sensor             # tracking error
        self.u = self.u + (self.Kp + self.Ki + self.Kd/self.dt) * error_e - (self.Kp + 2*self.Kd/self.dt) * self.e1 + self.Kd/self.dt * self.e2   # vl

        self.e2 = self.e1
        self.e1 = error_e

        # PID control with prediction attempt
        prediction_test = self.y[0] + self.u * self.dt

        self.y.append(prediction_test)
